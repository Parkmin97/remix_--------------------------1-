...
import { Activity, Clock, Play, ArrowRight, ShieldCheck, Target } from 'lucide-react';
import { TARGET_SERVICES } from '../data/targetServices';
import { TimeSlotPicker } from './TimeSlotPicker';

export const ModeBScreen: React.FC = () => {
  ...
  const [usageIntent, setUsageIntent] = useState<string>('업무 정보 탐색 및 답장');
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6 text-stone-100 pb-24">
      ...
      {isStarted ? (
        <div className="p-8 rounded-3xl bg-stone-900/90 border border-amber-500/40 text-center space-y-4 shadow-2xl">
          <div className="w-16 h-16 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/40 flex items-center justify-center mx-auto animate-pulse">
            <ShieldCheck className="w-8 h-8" />
          </div>
          <h2 className="text-xl font-bold font-serif text-amber-200">의도적 SNS 이용 세션이 시작되었습니다!</h2>
          <p className="text-xs text-stone-300">
            계획 이용: <strong className="text-amber-300 font-semibold">{usageLimit}분</strong> | 이용 목적: {usageIntent}
          </p>
          <p className="text-[11px] text-stone-400">
            이용 제한 종료 후 {focusDuration}분 집중 약속 세션이 자동 시작됩니다.
          </p>
          <button
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-stone-300 mb-2 flex items-center gap-1.5">
                  ...
                  <span>이용 후 집중 약속 시간 (5분 단위 슬롯 선택)</span>
                </label>
                <TimeSlotPicker
                  value={focusDuration}
            ...
            <div>
              ...
              <input
                ...
                placeholder="예: 중요 답장 및 업무 정보 탐색"
                className="w-full bg-stone-950 border border-amber-800/50 rounded-xl px-3 py-2.5 text-xs text-amber-100 focus:outline-none focus:border-amber-500"
              />
            </div>
