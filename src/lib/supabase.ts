...
export function toKoreanAuthError(message: string): string {
  ...
  if (m.includes('user not found')) return '가입되지 않은 이메일 주소입니다.';
  if (m.includes('password should be at least')) return '비밀번호는 최소 6자 이상이어야 합니다.';
  if (m.includes('same password') || m.includes('should be different')) return '기존 비밀번호와 다른 새 비밀번호를 입력해주세요.';
  if (m.includes('unable to validate email address') || m.includes('invalid email')) return '이메일 형식이 올바르지 않습니다.';
  if (m.includes('rate limit') || m.includes('too many') || m.includes('over_email_send_rate_limit')) return '이메일 발송 요청이 너무 많습니다. 잠시 후 다시 시도해주세요.';
  return message;
}
